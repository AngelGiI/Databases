...
/* 1. Consultas básicas */
-- Selección de columnas completa
SELECT * FROM donantes; 
-- Selección de columnas parcial (sin y con alias de columnas)
...
/* 2. Consultas con criterios de selección
   - Operadores de comparación: = (equal), <=> (NULL-safe equal), <> (not equal), != (not equal), <=, <, >, >=, 
		IS [NOT] valorBooleano ({TRUE, FALSE, UNKNOWN}), IS [NOT] NULL, expresión [NOT] IN (valor1,valor2,...valorN),
        expresión LIKE patrón (pattern matching)
...
SELECT COUNT(ID_organo)
   FROM organos;
-- Función agregada de aplicación conjunta con eliminación previa de repeticiones   
